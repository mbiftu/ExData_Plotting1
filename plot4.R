...
datetime <- strptime(paste(subsetorg$Date, subsetorg$Time, sep=" "), "%d/%m/%Y %H:%M:%S")  
 ...
 globalReactivePower <- as.numeric(subsetorg$Global_reactive_power) 
 voltage <- as.numeric(subsetorg$Voltage) 
 subMetering1 <- as.numeric(subsetorg$Sub_metering_1) 
 subMetering2 <- as.numeric(subsetorg$Sub_metering_2) 
 subMetering3 <- as.numeric(subsetorg$Sub_metering_3) 
